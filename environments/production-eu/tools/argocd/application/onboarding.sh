...
SERVICE_ACCOUNT_ANNOTATION=$(echo {"eks.amazonaws.com/role-arn": "arn:aws:iam::$ACCOUNT_ID:role/$APPLICATION_NAME-s3-role-k8s-$GIT_BRANCH"} )
ANNOTATION=$SERVICE_ACCOUNT_ANNOTATION yq -i e '.microservice.serviceAccount.annotations +=env(ANNOTATION) ' microservice.yaml
AFFINITY="$APPLICATION_NAME" yq -i e '.microservice.affinity.podAntiAffinity.preferredDuringSchedulingIgnoredDuringExecution[0].podAffinityTerm.labelSelector.matchExpressions[0].values[0]=env(AFFINITY) ' microservice.yaml
...
then
    ...
    yq -i e '.microservice.deployment.terminationGracePeriodSeconds=300 ' microservice.yaml
fi

if [ $PROJECT_TYPE == "ia" ]
then
    yq -i e '.microservice.deployment.terminationGracePeriodSeconds=330 ' microservice.yaml
fi
...
then
    ...
    yq -i e '.microservice.deployment.terminationGracePeriodSeconds=86400 ' microservice.yaml
    yq -i e '.microservice.deploymentFree.terminationGracePeriodSeconds=86400 ' microservice.yaml
fi

if [ $PROJECT_TYPE != "io" ]
then
    yq -i e 'del(.microservice.deploymentFree)' microservice.yaml
    yq -i e 'del(.microservice.hpaFree)' microservice.yaml
fi
